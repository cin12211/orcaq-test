...
export const rowQueryFilesElectronAdapter: RowQueryFilesPersistApi = {
  ...
  createFiles: async fileValue => {
    ...
    if (!existingContent) {
      await persistUpsert<RowQueryFileContent>('rowQueryFileContents', file.id, {
        id: file.id,
        contents: '',
        variables: '',
      });
  ...
  updateFile: async fileValue => {
    const existing = await persistGetOne<RowQueryFile>('rowQueryFiles', fileValue.id);
  ...
  updateFileContent: async fileContent => {
    const existing = await persistGetOne<RowQueryFileContent>('rowQueryFileContents', fileContent.id);
    if (!existing) return null;

    return persistUpsert<RowQueryFileContent>('rowQueryFileContents', fileContent.id, fileContent);
  },
  ...
  deleteFile: async ({ id }) => {
    await Promise.all([
      persistDelete<RowQueryFile>('rowQueryFiles', [{ field: 'id', value: id }], 'all'),
      persistDelete<RowQueryFileContent>('rowQueryFileContents', [{ field: 'id', value: id }], 'all'),
    ]);
  },

  deleteFileByWorkspaceId: async ({ wsId }) => {
    const files = await rowQueryFilesElectronAdapter.getFilesByContext({ workspaceId: wsId });
    await Promise.all(files.map(file => rowQueryFilesElectronAdapter.deleteFile({ id: file.id })));
  },
};
