...
export const quickQueryLogsElectronAdapter: QuickQueryLogsPersistApi = {
  getAll: async () => {
    return sortByCreatedAt(await persistGetAll<QuickQueryLog>('quickQueryLogs'));
  },
  ...
  update: async qqLog => {
    const existing = await persistGetOne<QuickQueryLog>('quickQueryLogs', qqLog.id);
  ...
  delete: async (props: DeleteQQueryLogsProps) => {
    ...
    if ('workspaceId' in props) filters.push({ field: 'workspaceId', value: props.workspaceId });
    if ('connectionId' in props) filters.push({ field: 'connectionId', value: props.connectionId });
    if ('schemaName' in props) filters.push({ field: 'schemaName', value: props.schemaName });
    if ('tableName' in props) filters.push({ field: 'tableName', value: props.tableName });
